...
Translation applies four general phases. The _extraction_ phase scans the original Parcel source files and extracts the parcel interface _declarations_, as well as each declaration's associated _export_ statement, and any parcel _import_ statements. The _analysis_ phase analyses the parcel declarations within the context of their enclosing C source code, constructing internal data structures representing Parcel language entities. It also analyses the relevant enclosing C code syntactic and semantic translation units, expressions, statements and functions. The _integration_ phase compares the Parcel and C language entities to calculate the integrated representation of their combined semantics. The analysis and integration phases interact iteratively, starting from the lowest and progressing to the highest levels of abstraction. For example, the translation first calculates intra-parcel properties such as the interface identifier definitions, and progresses through inter-parcel properties such as the parcels' export-import relationships. If analysis and integration determine that the declared parcels and their relationships are correctly and completely defined, then the _generation_ phase generates their modular semantic representations in ordinary C files, that the original source files `#include` through their respective export and import statements.
...
**Identifier kind mismatch.** An identifier listed under a `#pragma <kind>:` label has a C declaration whose kind conflicts with the stated kind. For example, listing a typedef identifier under `function:`, or a function identifier under `constant:`.
...
**Redeclared parcel.** A second or subsequent `#pragma parcel <name>` declaration appears for the same parcel `<name>` within the same translation unit.  
...
**Undefined parcel.** A file contains `#include "import/<path>/<name>.<stem>"` but with no corresponding `#include "export/..."` statement for that parcel name. The translator has no interface from which to derive the _import_ file.
...
**Exported static identifier.** An identifier listed in a `#pragma parcel` declaration is declared `static` in the same translation unit. Static identifiers have internal linkage and are not accessible to importers; the generated export file will compile but the exported name will be unusable in other translation units.
