...
**Duplicate parcel declaration.** The same parcel name appears in more than one `#pragma parcel` declaration within the same file. The interface is ambiguous.

**Unknown import.** A file contains `#include "import/<path>/<name>.<stem>"` but no `#pragma parcel <name>` declaration has been found at `<path>` in any scanned source file. The translator has no interface from which to derive the import file contents.
...
**Pragma without export include.** A `#pragma parcel` declaration has no corresponding `#include "export/..."` in the same file. The parcel is declared but never exported; this is unusual outside of interface-only or draft files.
